...
module token_v2::collection {
    ...
    use aptos_framework::object::{Self, ConstructorRef, Object, ExtendRef, TransferRef, DeleteRef, generate_extend_ref, generate_transfer_ref, generate_delete_ref, object_address};
    use aptos_std::smart_table::SmartTable;
    use aptos_std::smart_table;
    use token_v2::common::{Royalty, new_caps_from_constructor_ref, Caps, get_object_from_caps, extract_delete_from_caps};
    ...
    struct CollectionIndex has key {
        index: SmartTable<String, Caps<Collection>>
    }
    ...
    public fun create_fixed_collection(
        ...
        smart_table::add(collection_index, name, new_caps_from_constructor_ref<Collection>(&creator_ref));
    ...
    fun remove_collection_caps(creator: address, name: &String): Caps<Collection> acquires CollectionIndex {
    ...
    public fun get_collection_object(creator: address, name: String): Object<Collection> acquires CollectionIndex {
        ...
        let collection_index = &mut borrow_global_mut<CollectionIndex>(creator).index;
        assert!(smart_table::contains(collection_index, name), 0);
        get_object_from_caps(smart_table::borrow(collection_index, name))
    }
    ...
    public fun delete_collection(
        ...
        let collection_refs = remove_collection_caps(signer::address_of(creator), &name);
        let collection_addr = object_address(&get_object_from_refs(&collection_refs));
        if (exists<FixedSupply>(collection_addr)) {
        ...
        object::delete(extract_delete_from_caps(&mut collection_refs));
    }
