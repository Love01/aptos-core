...
module token_v2::token {
    ...
    const ETOKEN_EXISTENCE: u64 = 1;
    /// The provided signer is not the creator
    const ENOT_CREATOR: u64 = 2;
    /// Attempted to mutate an immutable field
    const EFIELD_NOT_MUTABLE: u64 = 3;
    /// The token indexer existence.
    const ETOKEN_INDEXER_EXISTENCE: u64 = 4;
    /// The existence of a specific token index in the token indexer.
    const ETOKEN_INDEX_EXISTENCE: u64 = 5;
    /// The provided signer is not the owner
    const ENOT_OWNER: u64 = 6;
    /// The current_supply of token as fungible assets is not zero.
    const ECURRENT_SUPPLY_NON_ZERO: u64 = 7;
    /// The ref does not match the token object
    const ETOKEN_OBJECT_AND_REF_MISMATCH: u64 = 8;
    /// The existence of extend_ref
    const EEXTEND_REF_EXISTENCE: u64 = 9;
    /// The existence of transfer_ref
    const ETRANSFER_REF_EXISTENCE: u64 = 10;
    /// The existence of extend_ref
    const EDELETE_REF_EXISTENCE: u64 = 11;
    /// The token is not an NFT
    const ENOT_NFT: u64 = 12;

    use aptos_framework::object::{Self, ConstructorRef, Object, create_object_from_account, object_from_extend_ref, generate_signer_for_extending, TransferRef, transfer, generate_linear_transfer_ref, transfer_with_ref, object_from_transfer_ref, object_address, object_from_delete_ref, delete, is_owner, ExtendRef, DeleteRef};

    use aptos_std::smart_vector::SmartVector;
    use aptos_std::smart_table::SmartTable;
    use token_v2::common::{Refs, Royalty, new_refs_from_constructor_ref, init_fungible_asset_data, get_current_supply, assert_valid_name, refs_contain_extend, extract_extend_from_refs, add_extend_to_refs, refs_contain_transfer, add_transfer_to_refs, extract_transfer_from_refs, refs_contain_delete, extract_delete_from_refs, add_delete_to_refs};
    use aptos_std::smart_table;
    use token_v2::collection::{Collection, get_collection_object};
    use token_v2::common;
    use token_v2::collection;
    use std::signer::address_of;
    use token_v2::coin::CashedCoin;
    use token_v2::coin;
    ...
    struct TokenIndexer has key {
        index: SmartTable<String, Refs<Token>>
    }
    ...
    public fun create_token(
        ...
        let creator_refs = new_refs_from_constructor_ref<Token>(&creator_ref, creator_enabled_refs);
        assert!(exists<TokenIndexer>(creator_address), error::not_found(ETOKEN_INDEXER_EXISTENCE));
        let token_indexer = &mut borrow_global_mut<TokenIndexer>(creator_address).index;
        assert!(!smart_table::contains(token_indexer, name), error::already_exists(ETOKEN_INDEX_EXISTENCE));
        smart_table::add(token_indexer, token_index_key, creator_refs);

        let owner_refs = OwnerRefs {
            refs: new_refs_from_constructor_ref<Token>( &creator_ref, owner_enabled_refs)
        };
    ...
    public fun convert_nft_to_ft(owner: &signer, token_obj: Object<Token>, coin_ref_flags: vector<bool>) {
        assert!(is_owner(token_obj, address_of(owner)), error::permission_denied(ENOT_OWNER));
        common::assert_fungible_asset_metadata_exists(token_obj);
        init_fungible_asset_data(owner, coin_ref_flags);
    }
    ...
    public fun convert_ft_into_nft(owner: &signer, token_obj: Object<Token>) {
        assert!(is_owner(token_obj, address_of(owner)), error::permission_denied(ENOT_OWNER));
        assert!(get_current_supply(token_obj) == 0, error::permission_denied(ECURRENT_SUPPLY_NON_ZERO));
        common::remove_fungible_asset_metadata(owner, token_obj);
    }

    /// If a `Token` is a fungible asset instead of NFT, this function could be called to mint its fungible token.
    public fun mint_fungible_coin(owner: &signer, token_obj: Object<Token>, amount: u64): CashedCoin<Token> {
        assert!(is_owner(token_obj, address_of(owner)), error::permission_denied(ENOT_OWNER));
        common::assert_fungible_asset_metadata_exists(token_obj);
        coin::mint(owner, token_obj, amount)
    }
    ...
    fun borrow_creator_refs(token_obj: Object<Token>): &Refs<Token> acquires Token, TokenIndexer {
        ...
        assert!(smart_table::contains(token_index, token_index_key), error::not_found(ETOKEN_INDEX_EXISTENCE));
        smart_table::borrow(token_index, token_index_key)
    }

    fun borrow_creator_refs_mut(token_obj: Object<Token>): &mut Refs<Token> acquires Token, TokenIndexer {
        let token_addr = verify(&token_obj);
        let token = borrow_global<Token>(token_addr);
        let collection_name = option::map(token.collection, |obj| collection::name(obj));
        let token_index_key = generate_token_index_key(collection_name, token.name);
    ...
    fun remove_creator_refs(token_obj: Object<Token>): Refs<Token> acquires Token, TokenIndexer {
        let token_addr = verify(&token_obj);
        let token = borrow_global<Token>(token_addr);
    ...
    public fun get_extend_ref_from_owner_refs(owner: &signer, token_obj: Object<Token>): ExtendRef acquires OwnerRefs {
        ...
        assert!(refs_contain_extend(owner_refs), error::not_found(EEXTEND_REF_EXISTENCE));
        extract_extend_from_refs(owner_refs)
    }

    public fun put_extend_ref_to_owner_refs(owner: &signer, token_obj: Object<Token>, extend_ref: ExtendRef) acquires OwnerRefs {
        assert_owner(owner, token_obj);
        let owner_refs = &mut borrow_global_mut<OwnerRefs>(object_address(&token_obj)).refs;
        assert!(object_from_extend_ref<Token>(&extend_ref) == token_obj, error::invalid_argument(ETOKEN_OBJECT_AND_REF_MISMATCH));
        assert!(!refs_contain_extend(owner_refs), error::already_exists(EEXTEND_REF_EXISTENCE));
    ...
    public fun get_extend_ref_from_creator_refs(creator: &signer, token_obj: Object<Token>): ExtendRef acquires Token, TokenIndexer {
        assert_creator(creator, token_obj);
        let creator_refs = borrow_creator_refs_mut(token_obj);
        assert!(refs_contain_extend(creator_refs), error::not_found(EEXTEND_REF_EXISTENCE));
        extract_extend_from_refs(creator_refs)
    }

    public fun put_extend_ref_to_creator_refs(creator:&signer, token_obj: Object<Token>, extend_ref: ExtendRef) acquires Token, TokenIndexer {
        assert_creator(creator, token_obj);
        let creator_refs = borrow_creator_refs_mut(token_obj);
        assert!(object_from_extend_ref<Token>(&extend_ref) == token_obj, error::invalid_argument(ETOKEN_OBJECT_AND_REF_MISMATCH));
        assert!(!refs_contain_extend(creator_refs), error::already_exists(EEXTEND_REF_EXISTENCE));
    ...
    public fun get_transfer_ref_from_owner_refs(owner: &signer, token_obj: Object<Token>): TransferRef acquires OwnerRefs {
        assert_owner(owner, token_obj);
        let owner_refs = &mut borrow_global_mut<OwnerRefs>(object_address(&token_obj)).refs;
        assert!(refs_contain_transfer(owner_refs), error::not_found(ETRANSFER_REF_EXISTENCE));
        extract_transfer_from_refs(owner_refs)
    }

    public fun put_transfer_ref_to_owner_refs(owner: &signer, token_obj: Object<Token>, transfer_ref: TransferRef) acquires OwnerRefs {
        assert_owner(owner, token_obj);
        let owner_refs = &mut borrow_global_mut<OwnerRefs>(object_address(&token_obj)).refs;
        assert!(object_from_transfer_ref<Token>(&transfer_ref) == token_obj, error::invalid_argument(ETOKEN_OBJECT_AND_REF_MISMATCH));
        assert!(!refs_contain_transfer(owner_refs), error::already_exists(ETRANSFER_REF_EXISTENCE));
    ...
    public fun get_transfer_ref_from_creator_refs(creator:&signer, token_obj: Object<Token>): TransferRef acquires Token, TokenIndexer {
        assert_creator(creator, token_obj);
        let creator_refs = borrow_creator_refs_mut(token_obj);

        assert!(refs_contain_transfer(creator_refs), error::not_found(ETRANSFER_REF_EXISTENCE));
        extract_transfer_from_refs(creator_refs)
    }

    public fun put_transfer_ref_to_creator_refs(creator:&signer, token_obj: Object<Token>, transfer_ref: TransferRef) acquires Token, TokenIndexer {
        assert_creator(creator, token_obj);
        let creator_refs = borrow_creator_refs_mut(token_obj);

        assert!(object_from_transfer_ref<Token>(&transfer_ref) == token_obj, error::invalid_argument(ETOKEN_OBJECT_AND_REF_MISMATCH));
        assert!(!refs_contain_transfer(creator_refs), error::already_exists(ETRANSFER_REF_EXISTENCE));
    ...
    public entry fun delete_token_from_owner_refs(owner: &signer, token_obj: Object<Token>) acquires OwnerRefs, Token, TokenIndexer {
        ...
        assert!(refs_contain_delete(owner_refs), error::not_found(EDELETE_REF_EXISTENCE));
    ...
    public fun put_delete_ref_to_owner_refs(owner: &signer, token_obj: Object<Token>, delete_ref: DeleteRef) acquires OwnerRefs {
        assert_owner(owner, token_obj);
        let owner_refs = &mut borrow_global_mut<OwnerRefs>(object_address(&token_obj)).refs;
        assert!(object_from_delete_ref<Token>(&delete_ref) == token_obj, error::invalid_argument(ETOKEN_OBJECT_AND_REF_MISMATCH));
        assert!(!refs_contain_delete(owner_refs), error::already_exists(EDELETE_REF_EXISTENCE));
    ...
    public entry fun delete_token_from_creator_refs(creator:&signer, token_obj: Object<Token>) acquires Token, TokenIndexer, OwnerRefs {
        // Deleting a token as a base of fungible asset is not allowed.
        assert_nft(token_obj);
        assert_creator(creator, token_obj);
        ...
        assert!(refs_contain_delete(&creator_refs), error::not_found(EDELETE_REF_EXISTENCE));
    ...
    public fun put_delete_ref_to_creator_refs(creator:&signer, token_obj: Object<Token>, delete_ref: DeleteRef) acquires Token, TokenIndexer {
        assert_creator(creator, token_obj);

        let creator_refs = borrow_creator_refs_mut(token_obj);
        assert!(object_from_delete_ref<Token>(&delete_ref) == token_obj, error::invalid_argument(ETOKEN_OBJECT_AND_REF_MISMATCH));
        assert!(!refs_contain_delete(creator_refs), error::already_exists(EDELETE_REF_EXISTENCE));
    ...
    inline fun verify<T: key>(token: &Object<T>): address {
        ...
        assert!(
            ...
            error::not_found(ETOKEN_EXISTENCE),
        );
        token_address
    }
